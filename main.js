...
// Add your functions below:
 const validateCred = arr => {
   ...
   /* Reverse numbers, remove the checkDigit and apply algorithm*/
   const checkSum = arr.slice()
   checkSum.reverse()
    const checkDigit = checkSum[0]
    checkSum.shift()
   ...
   for (let i = 0; i < checkSum.length; i++) {
    ...
    /* Add checkDigit back*/
    checkSum.unshift(checkDigit) 

    /* Add all numbers to find total sum */
    let totalSum = 0;
    for (let j = 0; j < checkSum.length; j++) {
      totalSum = totalSum + checkSum[j];
   }
   
   /* Modulo check to see if it's valid */
   if (totalSum%10 === 0) {
      return true;
...
console.log(invalidCompanies.join(", "));
